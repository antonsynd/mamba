...
#include "mamba/list2.hpp"
#include "mamba/sequence.hpp"

namespace mamba::builtins::types::v2::test {

template <typename T>
using ListHandle = List<T>::handle;

namespace {

template <typename T>
std::vector<T> as_vector(const ListHandle<T>& l) {
  std::vector<T> res;
...
TEST(List, VariadicConstructor) {
  // If/when
  const auto l = List<Int>::Init(1, 3, 5, 7);
...
TEST(List, InitializerListConstructor) {
  // If/when
  const auto l = List<Int>::Init(std::initializer_list<Int>{1, 3, 5, 7});

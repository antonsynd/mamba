...
struct Wrapper : public builtins::ObjectType {
 public:
  ...
  Wrapper() : data_(value_type(), GetNextId()) {}

  explicit Wrapper(value_type value) : data_(value, GetNextId()) {}
  ...
  static builtins::StrType __Name__() {
    std::ostringstream oss;
  ...
  bool operator<(const self& other) const { return data_.v_ < other.data_.v_; }
...
template <typename T, test::EqualityMethod EM>
struct Traits<test::Wrapper<T, EM>> {

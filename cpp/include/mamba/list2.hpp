...
#include "mamba/handle2.hpp"
#include "mamba/iteration.hpp"
#include "mamba/str.hpp"

namespace mamba::builtins::types::v2 {
namespace details {
...
class List : public std::enable_shared_from_this<List<T>> {
 public:
  ...
  using handle = memory::v2::Handle<self>;
 ...
 public:
  ...
  explicit List(Private, const I& it) {}

  /// @brief Creates a list with the provided variadic arguments.
  /// @code list(...)
  template <typename... Args>
  List(Private, T elem, Args... rest) {
    Append(elem);
    Append(rest...);
  }
  ...
  /// @code [...]
  List(Private, std::initializer_list<T> elements) {
  ...
  static handle Init(Args... args) {
    return memory::v2::Make<self>(Private(), std::forward<Args>(args)...);
  }
  ...
  void Extend(const handle& other) {
  ...
  /// @brief Extends this list with the elements of @p other.
  /// @code list += other
  void operator+=(const handle& other) { this->Extend(other); }
  ...
  /// @code list[i:j:k]
  handle Slice(Int start = 0, Int end = kEndIndex, Int step = 1) const {
    auto res = Init();
  ...
  handle operator[](Int start = 0, Int end = kEndIndex, Int step = 1) const {
  ...
  /// @todo
  /// @code list.__iter__()
  memory::v2::Handle<Iterator<T>> Iter() {
    return details::ListIterator<T>::Init(v_.begin(), v_.end());
  }
  ...
  /// @code list == other
  template <>
  Bool Eq(const handle& other) const {
    return v_ == other->v_;
  }
 ...
 private:
  ...
  void ReplaceSliceSingleStepExpanding(const handle& other,
                                       size_t start,
  ...
  void ReplaceSliceSingleStepReducing(const handle& other,
                                      iterator start_it,
                                      size_t num_old_elems,
                                      size_t num_new_elems) {
  ...
  void ReplaceSliceMultiStep(const handle& other,
                             SliceParams<iterator> slice_params) {
...
class ListIterator :
 ...
 private:
  ...
  ListIterator(Private, iterator it, iterator end)
 ...
 public:
  ...
  template <typename... Args>
  static handle Init(Args&&... args) {
    return Make<self>(Private(), std::forward<Args>(args)...);
  }

  handle Iter() {
    return std::enable_shared_from_this<ListIterator<T>>::shared_from_this();
  }
